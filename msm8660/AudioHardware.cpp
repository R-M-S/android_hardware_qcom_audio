        ...
        switch(temp_ptr->stream_type) {
            ...
            case PCM_REC:
                ...
                if(msm_route_stream(PCM_REC,temp_ptr->dec_id,DEV_ID(temp_ptr->dev_id),0)) {
                    ALOGV("msm_route_stream(PCM_REC,%d,%d,0) failed",temp_ptr->dec_id,DEV_ID(temp_ptr->dev_id));
                }
...
AudioHardware::AudioStreamOutMSM8x60::AudioStreamOutMSM8x60() :
    mHardware(0), mFdin(-1), mStartCount(0), mRetryCount(0), mStandby(true), mDevices(0)
{
}
...
{
    // ALOGD("AudioStreamOutMSM8x60::write(%p, %u)", buffer, bytes);
    status_t status = NO_INIT;
    size_t count = bytes;
    ...
    while (count) {
        ssize_t written = ::write(mFdin, p, count);
        if (written > 0) {
            count -= written;
            p += written;
        } else {
...
AudioHardware::AudioStreamOutDirect::AudioStreamOutDirect() :
    mHardware(0), mFdin(-1), mStartCount(0), mRetryCount(0), mStandby(true), mDevices(0),mChannels(AudioSystem::CHANNEL_OUT_MONO),
...
ssize_t AudioHardware::AudioStreamOutDirect::write(const void* buffer, size_t bytes)
{
    ...
    if (mStandby) {
        if(mHardware->mVoipFd >= 0) {
            ...
            mFdin = status;
            mHardware->mVoipFd = mFdin;
            // configuration
            ALOGV("get mvs config");
            struct msm_audio_mvs_config mvs_config;
            status = ioctl(mFdin, AUDIO_GET_MVS_CONFIG, &mvs_config);
            if (status < 0) {
            ...
            status = ioctl(mFdin, AUDIO_SET_MVS_CONFIG, &mvs_config);
            if (status < 0) {
            ...
            status = ioctl(mFdin, AUDIO_START, 0);
            if (status < 0) {
    ...
    while (count) {
        ...
        size_t written = ::write(mFdin, &audio_mvs_frame, sizeof(audio_mvs_frame));
...
#ifdef QCOM_VOIP
    ALOGV(" AudioStreamOutDirect::standby mHardware->mNumVoipStreams = %d mFdin = %d\n", mHardware->mNumVoipStreams, mFdin);
    if (mFdin >= 0 && (mHardware->mNumVoipStreams == 1)) {
       ...
       if (mFdin >= 0) {
           ret = ioctl(mFdin, AUDIO_STOP, NULL);
           ALOGE("MVS stop returned %d \n", ret);
           ::close(mFdin);
           mFdin = mHardware->mVoipFd = -1;
           ALOGV("driver closed");
           voip_session_id = 0;
           voip_session_mute = 0;
       }
...
AudioHardware::AudioStreamInVoip::AudioStreamInVoip() :
    mHardware(0), mFdin(-1), mState(AUDIO_INPUT_CLOSED), mRetryCount(0),
    mFormat(AUDIO_HW_IN_FORMAT), mChannels(AUDIO_HW_IN_CHANNELS),
    mSampleRate(AUDIO_HW_VOIP_SAMPLERATE_8K), mBufferSize(AUDIO_HW_VOIP_BUFFERSIZE_8K),
...
ssize_t AudioHardware::AudioStreamInVoip::read( void* buffer, ssize_t bytes)
{
  ...
  if(mFormat == AUDIO_HW_IN_FORMAT)
  {
      ...
      while (count >= mBufferSize) {
          ...
          int bytesRead = ::read(mFdin, &audio_mvs_frame, sizeof(audio_mvs_frame));
          ALOGV("read_bytes = %d \n", bytesRead);
          if (bytesRead > 0) {
